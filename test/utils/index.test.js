...
describe("Utils", () => {
  describe(".contains", () => {
    ...
    test.each(testRows)(
      "returns true when array contains given value: %s",
      (value) => {
    ...
    test.each(testRows)(
      "returns false when array does not contain given value: %s",
      (value) => {
  ...
  describe(".determinePropertyMessage", () => {
...

    {
      ...
      test("returns custom error message when it is passed one", () => {
        expect(determinePropertyMessage(date, "length", errorMessage)).toBe(
          errorMessage
        );
      });
    }
